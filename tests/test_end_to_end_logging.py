...
from lazyscan.security.safe_delete import get_safe_deleter, DeletionMode
from lazyscan.security.sentinel import initialize_sentinel, get_sentinel
...
class TestEndToEndLoggingIntegration:
    ...
    def test_complete_logging_pipeline(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w+', suffix='.json', delete=False) as main_log, \
            ...
            with profile_operation(logger, "cache_analysis"):
                logger.debug("Analyzing cache structure",
                            cache_count=42,
                            total_size_mb=256.8)
            ...
            error_entries = [entry for entry in main_lines 
                           if json.loads(entry)['level'] == 'ERROR']
            ...
            security_events = [json.loads(line) for line in audit_lines 
            ...
            deletion_events = [json.loads(line) for line in audit_lines 
    ...
    def test_error_reporting_with_audit_trail(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w+', suffix='.json', delete=False) as main_log, \
            ...
            error_entries = [json.loads(line) for line in main_lines 
