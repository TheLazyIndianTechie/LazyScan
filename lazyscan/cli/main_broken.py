...
# from ..apps.unity import handle_unity_discovery  # TODO: Fix syntax errors
# from ..apps.unreal import handle_unreal_discovery  # TODO: Fix syntax errors
# from ..apps.browser import handle_browser_discovery  # TODO: Fix syntax errors
# from ..apps.chrome import handle_chrome_discovery  # TODO: Fix syntax errors

# Import security framework
# from helpers.audit import audit_logger  # TODO: Fix syntax errors
# from helpers.secure_operations import configure_security  # TODO: Fix syntax errors
# from helpers.recovery import get_recovery_stats, register_operation_for_recovery  # TODO: Fix syntax errors
...
def scan_command(
    ...
    unreal_include_binaries: bool = typer.Option(False, "--unreal-include-binaries", help="include Binaries directories in Unreal cleanup (risky - requires full rebuild)"),
    unreal_include_marketplace: bool = typer.Option(False, "--unreal-include-marketplace", help="include Epic Marketplace cache in Unreal cleanup"),
    browser: bool = typer.Option(False, "--browser", help="scan browser caches (Firefox, Safari, Edge, Brave)"),
    ...
    class Args:
        def __init__(self):
            ...
            self.unity = unity
            self.unreal = unreal
            self.unreal_include_binaries = unreal_include_binaries
            self.unreal_include_marketplace = unreal_include_marketplace
...
permanent: bool = typer.Option(False, "--permanent", help="permanently delete instead of moving to trash"),
    # Developer tool specific options
    include_extensions: bool = typer.Option(False, "--include-extensions", help="Include extension caches (VS Code)"),
    reset_simulators: bool = typer.Option(False, "--reset-simulators", help="Reset Xcode simulator devices"),
    include_sdk: bool = typer.Option(False, "--include-sdk", help="Include Android SDK caches (Android Studio)"),
    include_avd: bool = typer.Option(False, "--include-avd", help="Include Android Virtual Devices (Android Studio)"),
    safety_level: str = typer.Option("safe", "--safety-level", help="Safety level for cleaning (safe/warn/danger/critical)"),
):
    """Clean cache directories and temporary files."""
    # Validate target
    valid_targets = ["macos", "unity", "chrome", "firefox", "vscode", "xcode", "androidstudio", "system", "all"]
    ...
    class Args:
        def __init__(self):
            ...
            self.reset_simulators = reset_simulators
            self.include_sdk = include_sdk
            self.include_avd = include_avd
...
operation_id: Optional[str] = typer.Option(None, "--operation-id", help="specific operation ID to restore"),
    decrypt_logs: bool = typer.Option(False, "--decrypt-logs", help="decrypt encrypted audit logs"),
log_file: Optional[str] = typer.Option(None, "--log-file", help="specific log file to decrypt"),
):
    """Manage file recovery and audit logs."""
    valid_actions = ["list", "restore", "stats", "logs", "decrypt"]
    ...
    self.decrypt_logs = decrypt_logs
...
app: Optional[str] = typer.Option(None, "--app", help="application-specific configuration (for per-app overrides)"),
    validate: bool = typer.Option(False, "--validate", help="validate configuration after changes"),
    preview_theme: Optional[str] = typer.Option(None, "--preview-theme", help="preview a theme configuration"),
    format: str = typer.Option("table", "--format", help="output format (table, json, toml)"),
):
    ...
    valid_actions = ["get", "set", "list", "reset", "info", "validate", "preview"]
    if action not in valid_actions:
        typer.echo(f"Error: Invalid action '{action}'. Valid actions: {', '.join(valid_actions)}")
        raise typer.Exit(1)
    ...
    try:
        if action == "get":
            ...
            if preview_theme:
                handle_theme_preview(preview_theme, format)
            else:
                typer.echo("Error: --preview-theme is required for preview action")
                raise typer.Exit(1)
...
def handle_config_validation():
    ...
    from ..core.config import PYDANTIC_AVAILABLE, LazyScanConfigModel, get_config

    typer.echo("=== Configuration Validation ===")

    if not PYDANTIC_AVAILABLE:
        typer.echo("❌ Pydantic not available - cannot perform validation")
        return

    try:
        config_dict = get_config()
        validated_config = LazyScanConfigModel.from_dict(config_dict)

        typer.echo("✅ Configuration is valid")
        typer.echo(f"   Theme: {validated_config.general.theme}")
        typer.echo(f"   Bar width: {validated_config.general.bar_width}")
        typer.echo(f"   Cache cleanup enabled: {validated_config.security.cache_cleanup_enabled}")

        # Check for any warnings
        if validated_config.security.docker_cleanup_enabled and not validated_config.security.allow_admin_operations:
            typer.echo("⚠️  Warning: Docker cleanup enabled but admin operations not allowed")
...
def handle_theme_preview(theme_name: str, output_format: str):
    ...
    if theme_name not in themes:
        typer.echo(f"Error: Theme '{theme_name}' not found")
        available = list(themes.keys())
        if available:
            typer.echo(f"Available themes: {', '.join(available)}")
        raise typer.Exit(1)
    ...
    if output_format == "json":
        import json
        typer.echo(json.dumps(theme, indent=2))
    elif output_format == "toml":
        try:
            import tomli_w
            # Convert to TOML structure
            toml_data = {f"themes.{theme_name}": theme}
            typer.echo(tomli_w.dumps(toml_data).strip())
        except ImportError:
            typer.echo("TOML output requires tomli_w (pip install tomli_w)")
    else:  # table format
        typer.echo(f"=== Theme Preview: {theme_name} ===")
        typer.echo()

        # Show logo if available
        if "logo" in theme and theme["logo"]:
            typer.echo("Logo:")
        ...
        typer.echo("Colors:")
        color_names = ["primary", "accent", "warning", "success"]
        for color_name in color_names:
            if color_name in theme:
                color_code = theme[color_name]
                sample_text = f"[{color_name}]"
                typer.echo(f"  {color_name}: {color_code}{sample_text}{colors[3]}")
        typer.echo()

        # Show glyphs
        if "glyphs" in theme:
            typer.echo("Glyphs:")
            for glyph_name, glyph_char in theme["glyphs"].items():
                typer.echo(f"  {glyph_name}: {glyph_char}")
            typer.echo()

        # Show animations
        if "animations" in theme:
            typer.echo("Animations:")
            for anim_name, anim_value in theme["animations"].items():
                typer.echo(f"  {anim_name}: {anim_value}")
...
def handle_scan_command(args):
    ...
    if args.browser:
        logger.info("Starting multi-browser discovery")
        handle_browser_discovery(args)
        return
...
def handle_clean_command(args):
    ...
    if args.target == "macos":
        ...
        # Execute clean operation with developer tool options
        clean_kwargs = {
            "dry_run": args.dry_run,
            "force": args.force,
            "permanent": args.permanent,
            "safety_level": getattr(args, 'safety_level', 'safe'),
        }

        # Add developer tool specific options
        if hasattr(args, 'include_extensions') and args.include_extensions:
            clean_kwargs["include_extensions"] = args.include_extensions
        if hasattr(args, 'reset_simulators') and args.reset_simulators:
            clean_kwargs["reset_simulators"] = args.reset_simulators
        if hasattr(args, 'include_sdk') and args.include_sdk:
            clean_kwargs["include_sdk"] = args.include_sdk
        if hasattr(args, 'include_avd') and args.include_avd:
            clean_kwargs["include_avd"] = args.include_avd

        result = plugin_manager.clean_with_plugin(args.target, **clean_kwargs)
...
def handle_decrypt_command(args):
    """Handle decryption of encrypted audit logs."""
    try:
        from lazyscan.security.recovery_decryptor import RecoveryDecryptor

        # Determine which log file to decrypt
        if args.log_file:
            log_file = Path(args.log_file)
            if not log_file.exists():
                console.print(f"Error: Log file {log_file} does not exist")
                return
            log_files = [log_file]
        else:
            ...
            from pathlib import Path
            import os
            log_dir = Path.home() / ".config" / "lazyscan" / "logs"
            if not log_dir.exists():
                console.print(f"Error: No audit logs found in {log_dir}")
                return
            log_files = list(log_dir.glob("*.jsonl"))

        if not log_files:
            console.print("No audit log files found")
            return

        # Create decryptor
        decryptor = RecoveryDecryptor()

        console.print(f"🔐 Decrypting {len(log_files)} audit log file(s)...")
        ...
        for log_file in log_files:
            try:
                console.print(f"Processing {log_file.name}...")

                # Decrypt the log file
                result = decryptor.recover_from_file(str(log_file))

                if result.success and result.decrypted_entries:
                    console.print(f"  ✅ Decrypted {len(result.decrypted_entries)} events")

                    # Display summary
                    event_types = {}
                    for event in result.decrypted_entries:
                    ...
                    total_decrypted += len(result.decrypted_entries)
                else:
                    console.print(f"  ⚠️  No encrypted events found or decryption failed")
                    if result.error_summary:
                        console.print(f"     Errors: {result.error_summary}")
                    total_failed += 1

            except Exception as e:
                console.print(f"  ❌ Failed to decrypt {log_file.name}: {e}")
                total_failed += 1

        console.print("n📊 Decryption Summary:")        console.print(f"  Total events decrypted: {total_decrypted}")
        console.print(f"  Files processed: {len(log_files)}")
        console.print(f"  Failed files: {total_failed}")

        if total_decrypted > 0:
            console.print("
💡 Tip: Use 'lazyscan recovery logs' to view decrypted audit summaries")
            console.print("n💡 Tip: Use 'lazyscan recovery logs' to view decrypted audit summaries")💡 Tip: Ensure audit encryption is enabled and logs exist")

    except ImportError:
        console.print("Error: Audit decryption not available. Encryption support may not be installed.")
    except Exception as e:
        console.print(f"Error during decryption: {e}")
