...
class ProgressDisplay:
    ...
    def update_progress(
        ...
        should_update = force_update or (current_time - self.last_update_time) >= self.update_interval
        ...
        if should_update or current == total:
            ...
            if extra_info:
                progress_line = f"{message}: [{bar}] {percent}% | {current}/{total} | {extra_info}"
            else:
...
def format_file_table_header(width: int, colors: Tuple[str, ...]) -> str:
    ...
    header = f"{BOLD}{ACCENT_COLOR}┌─{'─'*2}──{'─'*(width+2)}──{'─'*10}──{'─'*30}─┐{RESET}\n"
...
def format_file_table_footer(width: int, colors: Tuple[str, ...]) -> str:
    ...
    return f"{BOLD}{ACCENT_COLOR}└─{'─'*2}──{'─'*(width+2)}──{'─'*10}──{'─'*30}─┘{RESET}"
