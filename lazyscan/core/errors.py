...
def handle_exception(
    exc: Exception, logger, operation: str = "unknown", re_raise: bool = True
) -> dict[str, Any]:
    """
    ...
    if isinstance(exc, LazyScanError):
        ...
        try:
            ...
            if isinstance(exc, (SecurityPolicyError, DeletionSafetyError)):
                ...
                audit_data = {
                    k: v for k, v in exc_data.items() if k not in ["operation"]
                }
                log_security_event(
            ...
            logger.error(
                f"Operation '{operation}' failed: {exc.user_message}", extra=exc_data
            )
        ...
        try:
            ...
            structured_logger.error(
                f"Unexpected error in operation '{operation}'", **log_data
            )
        except ImportError:
            # Fallback to standard logging
            logger.error(
