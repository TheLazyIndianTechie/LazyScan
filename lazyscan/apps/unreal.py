...
def handle_unreal_discovery(args) -> None:
    ...
    logger.info("Starting Unreal Engine project discovery", extra={
        "operation": "unreal_discovery"
    })
    ...
    def discover_projects():
        ...
        try:
            ...
            logger.info("Unreal project discovery completed", extra={
                "project_count": len(projects)
            })
        except Exception as e:
            logger.error("Unreal project discovery failed", extra={
                "error": str(e)
            })
            projects = []
        discovery_done = True
    ...
    if not projects:
        console.print(f"{BOLD}{CYAN}[{YELLOW}!{CYAN}]{RESET} {YELLOW}No Unreal Engine projects found.{RESET}")
        logger.warning("No Unreal Engine projects found")
        return

    console.print(f"{BOLD}{CYAN}[{BRIGHT_MAGENTA}✓{CYAN}]{RESET} {GREEN}Found {len(projects)} Unreal Engine projects.{RESET}")

    # Prompt user to select projects
    if sys.stdin.isatty():
        selected_projects = prompt_unity_project_selection(projects)  # Reuse Unity selection logic
    else:
    ...
    if not selected_projects:
        console.print(f"{BOLD}{CYAN}[{YELLOW}!{CYAN}]{RESET} {YELLOW}No Unreal projects selected.{RESET}")
    ...
    logger.info("Starting Unreal project report generation", extra={
    ...
    for project in selected_projects:
        logger.debug("Processing Unreal project", extra={
        ...
        if sys.stdin.isatty() and report['total_size'] > 0:
            ...
            if response == 'y':
                logger.info("User chose to clean Unreal project caches", extra={
                ...
                logger.info("User selected Unreal cache cleanup options", extra={
                    "project_name": report['name'],
                    "choices": choices
                })
                ...
                if 'n' in choices:
                    console.print(f"{YELLOW}Skipping cache cleaning for this project.{RESET}")
                    logger.info("User skipped cache cleaning for project", extra={
                        "project_name": report['name']
                    })
                    continue
                ...
                for cache_name in directories_to_clear:
                    ...
                    try:
                        ...
                        logger.info("Starting Unreal cache deletion", extra={
                            "project_name": report['name'],
                        ...
                        if result.success:
                            ...
                            console.print(f"  {GREEN}✓ Cleared:{RESET} {cache_name} ({BRIGHT_MAGENTA}{human_readable(size_before)}{RESET} freed)")
                            logger.info("Unreal cache deletion successful", extra={
                                "project_name": report['name'],
                                "cache_name": cache_name,
                            ...
                            if "permission" in error_msg.lower():
                                console.print(f"  {RED}✗ Permission denied:{RESET} {cache_name}")
                            elif "not found" in error_msg.lower():
                                console.print(f"  {YELLOW}✗ Already deleted:{RESET} {cache_name}")
                            else:
                                console.print(f"  {RED}✗ Failed to clear {cache_name}:{RESET} {error_msg}")
                            logger.error("Unreal cache deletion failed", extra={
                                "project_name": report['name'],
                                "cache_name": cache_name,
                                "error": error_msg
                            })
                    except Exception as e:
                        console.print(f"  {RED}✗ Failed to clear {cache_name}:{RESET} {str(e)}")
                        logger.error("Unreal cache deletion exception", extra={
                            "project_name": report['name'],
                            "cache_name": cache_name,
                            "error": str(e)
                        }, exc_info=True)
                ...
                if project_freed > 0:
                    ...
                    logger.info("Unreal project cleanup completed", extra={
                        "project_name": report['name'],
                        "space_freed": project_freed
                    })

    console.print(f"\n{BOLD}{CYAN}[{BRIGHT_MAGENTA}SUMMARY{CYAN}]{RESET}")
    console.print(f"{CYAN}Total Cache Size for all selected projects:{RESET} {BRIGHT_MAGENTA}{human_readable(total_size)}{RESET}")
    if total_freed > 0:
        console.print(f"{GREEN}Total Space Freed:{RESET} {BRIGHT_MAGENTA}{human_readable(total_freed)}{RESET}")
    
    logger.info("Unreal Engine discovery and cleanup completed", extra={
