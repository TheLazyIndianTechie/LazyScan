...
from ..core.ui import get_console, knight_rider_animation
...
def handle_chrome_discovery(args) -> None:
    ...
    try:
        ...
        logger.error(
            "Chrome cache analysis failed", extra={"error": str(e)}, exc_info=True
        )
        console.print(
            f"{BOLD}{CYAN}[{RED}!{CYAN}]{RESET} {RED}Chrome cache analysis failed: {e!s}{RESET}"
        )
        return
