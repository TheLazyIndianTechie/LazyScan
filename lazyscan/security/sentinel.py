...
from ..core.errors import SecurityPolicyError, PathValidationError
...
class SecurityPolicy:
    ...
    def _validate(self):
        ...
        required_sections = ['behavior_flags', 'size_limits', 'allowed_roots', 'deny_patterns']
        ...
        required_flags = ['require_trash_first', 'interactive_double_confirm', 'block_symlinks']
        for flag in required_flags:
            if flag not in self.behavior_flags:
                raise SecurityPolicyError(f"Policy missing required behavior flag: {flag}")
        
        # Validate size limits
        required_limits = ['large_directory_threshold_mb', 'max_deletion_size_mb']
        for limit in required_limits:
            if limit not in self.size_limits:
                raise SecurityPolicyError(f"Policy missing required size limit: {limit}")
...
class SecuritySentinel:
    ...
    def guard_delete(self, path: Path, context: str = "general", operation_mode: str = "trash") -> None:
        """
        ...
        logger.debug(f"Guarding delete operation: {path} (context: {context}, mode: {operation_mode})")
        ...
        try:
            ...
            if operation_mode == "permanent" and self.policy.should_require_trash_first():
                raise SecurityPolicyError(
...
def guard_delete(path: Path, context: str = "general", operation_mode: str = "trash") -> None:
    """
...
def startup_health_check() -> None:
    ...
    try:
        ...
        console.print_error("LazyScan cannot start safely. Please check logs and configuration.")
        sys.exit(1)
...
try:
    ...
    if not _default_policy_path.exists():
        raise SecurityPolicyError("Default policy file missing - security system unavailable")
