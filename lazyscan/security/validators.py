    ...
    if os.name == 'nt':
        ...
        for part in parts:
            ...
            if name in WINDOWS_RESERVED_NAMES:
                raise PathValidationError(f"Path contains Windows reserved name: {name}")
        
        # Block trailing dots or spaces in components
        for part in path.parts:
            if part.endswith('.') or part.endswith(' '):
                raise PathValidationError(f"Path component ends with dot/space: {part!r}")
...
def validate_user_supplied_path(path: Union[str, Path], context: str = "general") -> Path:
    """
